...
#define MMS_VSC_CMD_SELFTEST		0X08
...
struct mms_ts_info {
	...
	bool ft_flag;
	bool if_con_sense_flag;
#endif /* SEC_TSP_FACTORY_TEST */
};

static int get_if_con_sense_state(void)
{
	return (gpio_get_value_cansleep(
		PM8921_GPIO_PM_TO_SYS(28))) ^ 1;
}
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	if (ret != ARRAY_SIZE(msg)) {
		dev_err(&client->dev,
			"failed to read %d bytes of touch data (%d), mms reset\n",
			sz, ret);
		goto out;
	}
	...
	if (buf[0] == 0xF) {
		dev_dbg(&client->dev, "Burst / ESD detected\n");
		if (info->if_con_sense_flag) {
			printk(KERN_ERR "PBA state, no reset!!");
			return IRQ_HANDLED;
		} else {
			printk(KERN_ERR "ESD detected, reset!!");
			goto err_i2c_fail;
		}
	}
	...
	for (i = 0; i < sz; i += FINGER_EVENT_SZ) {
		...
		if (id >= MAX_FINGERS) {
			dev_dbg(&client->dev,
			"is not normal coordinate, mms reset [%d]\n", id);
			goto err_not_normal;
		}
	...
	for (i = 0; i < MAX_FINGERS; i++) {
		if (info->finger_state[i] == 1)
			info->touch_is_pressed++;
	}

#if TOUCH_BOOSTER
	set_dvfs_lock(info, !!info->touch_is_pressed);
#endif
...
static int mms_ts_fw_load(struct mms_ts_info *info)
{
	...
	if (module_rev < 0) {
		module_rev = 0;
		dev_err(&client->dev,
			"can't read revision,controller dead? FW update!\n");
	}
...
static void get_raw_data_all(struct mms_ts_info *info, u8 cmd)
{
	...
	for (i = 0; i < RX_NUM; i++) {
		for (j = 0; j < TX_NUM; j++) {
			...
			if (i == 0 && j == 0)
				max_val = min_val = raw_data;

			max_val = max(max_val, raw_data);
			min_val = min(min_val, raw_data);
	...
	snprintf(buff, sizeof(buff), "%d,%d", min_val, max_val);
...
static int check_rx_tx_num(void *device_data)
{
	...
	if (info->cmd_param[0] < 0 ||
			info->cmd_param[0] >= RX_NUM  ||
			info->cmd_param[1] < 0 ||
			info->cmd_param[1] >= TX_NUM) {
		...
		*devattr, char *buf)
{
	...
	for (i = 0; i < RX_NUM; i++) {
		for (j = 0; j < TX_NUM; j++) {
			...
			if (ret < 0)
				goto err_i2c;
			reference = (((u16)read_buffer[1] << 8) |
							read_buffer[0]) >> 3;
			dev_dbg(&info->client->dev, "[TSP] reference[%d][%d] = %d\n",
							i, j, reference);

			ftn_ret += sprintf(buf+ftn_ret, "%3d %3d\n",
							intensity, reference);
		}
	}
...
err_i2c:
	dev_err(&info->client->dev, "%s: fail to i2c\n",
			__func__);
	return 0;
}
...
static DEVICE_ATTR(raw_data, S_IRUGO, show_raw_data, NULL);
#ifdef ESD_DEBUG
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (!input_dev) {
		dev_err(&client->dev, "Failed to allocate memory for intput device\n");
		goto err_alloc;
	}
	...
	if (get_if_con_sense_state()) {
		info->if_con_sense_flag = true;
		printk(KERN_ERR "if_con_sense_flag = true");
	} else {
		info->if_con_sense_flag = false;
		printk(KERN_ERR "if_con_sense_flag = false");
	}
#endif
